...
class Evaluator(object):
    ...
    def Pixel_Precision_Rate(self):
        ...
        Pre = self.confusion_matrix[1, 1] / (self.confusion_matrix[0, 1] + self.confusion_matrix[1, 1]+eps)
        return Pre

    def Pixel_Recall_Rate(self):
        eps = 1e-6
        assert self.confusion_matrix.shape[0] == 2
        Rec = self.confusion_matrix[1, 1] / (self.confusion_matrix[1, 0] + self.confusion_matrix[1, 1]+eps)
        return Rec
    ...
    def Intersection_over_Union(self):
        ...
        IoU = self.confusion_matrix[1, 1] / (
                self.confusion_matrix[0, 1] + self.confusion_matrix[1, 0] + self.confusion_matrix[1, 1]+eps)
        return IoU
    ...
    def add_batch(self, gt_image, pre_image):
        ...
        self.confusion_matrix += self._generate_matrix(gt_image, pre_image) #断言gt和pre形状是否相同，若相同则生成一个两者的的混淆矩阵
                # 调试输出
